...
def reset_count(previous_degree=0):
    """
...
def dambbell_direction(frame, direction):
...
def count_dambbell(frame ,degree, direction, min_degree, max_degree , count=0, upward=False,
    ...
    print(direction)
    frame = dambbell_direction(frame, direction)

    cv2.putText(frame, "{}".format(int(count)) ,(100, 100), 
                cv2.FONT_HERSHEY_PLAIN, 4, (0, 255, 0), 4)
    # print("Downward : ", downward)
    # print("Upward : ", upward)
    return frame, direction, upward, downward, count
...
def track_pose(frame, h, w, side="left"):
